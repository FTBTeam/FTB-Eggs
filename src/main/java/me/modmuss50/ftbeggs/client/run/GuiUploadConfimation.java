...
public class GuiUploadConfimation extends GuiScreen {
	...
	public void actionPerformed(GuiButton guiButton) throws IOException {
		...
		if (guiButton.id == CANCEL_BUTTON_ID) {
			...
			Minecraft.getMinecraft().displayGuiScreen(new GuiPostUpload(parent, RunManger.postData(data), "Achievement submitted!"));
		}
	}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		this.drawCenteredString(this.fontRenderer, "You WILL be able to re-submit this run later with a video.", this.width / 2, this.height / 2 + 24, -1);
	}
