...
public class GuiUpload extends GuiScreen {
	...
	public void actionPerformed(GuiButton guiButton) throws IOException {
		...
		if (guiButton.id == UPLOAD_BUTTON_ID) {
			...
			rdata.modFingerprint = "8727a3141c8ec7f173b87aa78b9b9807867c4e6b"; //TODO remove
			...
			if (rdata.videoURL == null || rdata.videoURL.isEmpty()) {
				...
				Minecraft.getMinecraft().displayGuiScreen(new GuiPostUpload(parent, RunManger.postData(rdata), "Achievement submitted!"));
			}
		}
	}

	public static String postDataToServer(RunData data) throws IOException {
		if (data.players == null || data.players.isEmpty()) {
			data.players = new ArrayList<>();
			data.players.add(new RunData.PlayerData(data.userName, data.uuid));
		}
		data.videoURL = "https://ftb.world/";
		data.modFingerprint = getModFingerprint();
		data.modFingerprint = "8727a3141c8ec7f173b87aa78b9b9807867c4e6b"; //TODO remove

		return RunManger.postData(data);
	}

	private static String getModFingerprint() {
		ModContainer container = null;
		for (ModContainer modContainer : Loader.instance().getModList()) {
			if (modContainer.getModId().equals("ftbeggs")) {
