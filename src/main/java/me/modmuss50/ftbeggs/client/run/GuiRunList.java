...
import me.modmuss50.ftbeggs.api.RunStatusManager;
import me.modmuss50.ftbeggs.api.SavedRunManager;
import me.modmuss50.ftbeggs.api.SecretManager;
...
public class GuiRunList extends GuiScreen {
	...
	public void initGui() {
		...
		RunStatusManager.reset();
		SecretManager.load();
	}

	private void initSaveList() {
		this.runList = SavedRunManager.getRuns();
	...
	public void drawScreen(int x, int y, float f) {
		...
		this.drawCenteredString(this.fontRenderer, TextFormatting.BLUE + "F" + TextFormatting.GREEN + "T" + TextFormatting.RED + "B" + TextFormatting.WHITE + " Eggs", this.width / 2, 20, -1);
	...
	private class RunList extends GuiSlot {
		...
		protected void elementClicked(int slot, boolean doubleClicked, int mouseX, int mouseY) {
			...
			RunData runData = getRun();
			if(SecretManager.hasSecrete(runData.runHash)){
				upload.displayString = "Add video";
			} else {
				upload.displayString = "Upload";
			}
		}
		...
		protected void drawSlot(int slot, int x, int y, int slotHeight, int mouseX, int mouseY, float ticks) {
			...
			String displayName = TextFormatting.GREEN + TimerServerHandler.getNiceTimeFromLong(runData.totalTime);
			String by = runData.userName;
			String topLine = displayName + TextFormatting.RESET + ", " + by;

			String date = runData.runDate;
			String middleLine = "(" + date.replace("T", " ") + ") ";

			String bottomLine = "Found " + TextFormatting.AQUA + runData.achievementData.size() + TextFormatting.RESET + " eggs";
			...
			bottomLine = bottomLine + " " + RunStatusManager.getRunColor(runData.runHash) + RunStatusManager.getRunStatus(runData.runHash);
	...
	private void uploadMap() {
		...
		RunData runData = getRun();
		if(!SecretManager.hasSecrete(runData.runHash)){
			Minecraft.getMinecraft().displayGuiScreen(new GuiUpload(parent, runData));
		} else {
			Minecraft.getMinecraft().displayGuiScreen(new GuiEditRun(parent, runData));
		}
